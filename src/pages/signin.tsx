...
const SignIn = () => {
    ...
    return (
        <div>
            <PageTitle title={"BETATRUS | SIGN IN"} />
            <AuthHeader />
            <main className="w-full justify-center items-center flex flex-col">
                ...
                <section className="lg:w-[60%] md:w-[80%] w-[90%] lg:p-15 md:px-10 py-15 px-5 rounded-2xl shadow-2xl text-[#1A1C1A]">
                    ...
                    <form action="post" className="space-y-5">
                        ...
                        <p className="text-[15.26px] text-primary leading-5 font-manrope text-end cursor-pointer" onClick={() => navigate("../reset_password")}>Forgot Password?</p>
                        <Button label="Sign In" extra="bg-primary w-full rounded-none py-[10px] text-manrope text-[16px] leading-6 text-white mt-5" />
                        <p className="font-manrope md:text-[15.26px] text-[12px] leading-[20.4px] tracking-[1.53px] text-center text-[##02102D]">Don’t have an account? <span className="cursor-pointer" onClick={() => navigate("../register")}>Sign up</span></p>
                    </form>
                </section>
                <AuthWriteup />
            </main>
